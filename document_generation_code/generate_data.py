...
def generate_data(company, Department, n, document_form, api_key = WW_GPT_API_KEY):
    ...
    * 생성해야 하는 문서는 "{Department}"부서 중 [기술부]대상 문서 이며 해당 부서의 *2개*의 문서를 다양한 문서 종류로 생성 해야해.

        <추가정보>
    개인정보 : 단 하나만으로도 특정 개인을 지칭하거나 추정할 수 있는 정보(이름, 주소, 주민번호 등)
    준식별자 : 하나로는 특정 개인을 지칭할 수 없지만 여러 준식별정보가 결합하는 경우나, 소량의 준식별정보와 주변에 개인정보가 분포하는 경우 특정 개인을 지칭하거나 추정이 가능한 정보(직업, 시군구 주소, 신체관련정보, 생일 등)
    ...
    *생략 없이 *2개*의 문서를 다 생성해줘.
    *문서명은 그동안 출력한 문서명 값과 겹치지 않게 해줘 
    *문서 하나를 생성하면 그 문서 안에 있는 개인정보와 준식별자 정보를 찾아서 정리할거야.
    *정리 양식은 "단어", "부서명", "문서명", "단어 유형", "개인정보/준식별자"를 칼럼으로 하는 csv 형식 표에 맞춰서 정리하면 돼. 
...
n = 5      # 전체 부서 개수
form_log = generate_data(company, Department, n, document_form, api_key=WW_GPT_API_KEY)
...
HEADER = ["단어","부서명","문서명","단어 유형","개인정보/준식별자"]
...
if all_tables:
    ...
    csv_path = OUT_DIR / "all_tables_기술부_9.csv"
